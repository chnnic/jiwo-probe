...
主题菜单中的 **Windows 2000** 是对 `guboysky/win2000` 视觉思路的原生适配：保留当前探针的 ProbeHub 数据、三组延迟、网速单位、流量详情和节点详情能力，页面呈现为桌面窗口、蓝色渐变标题栏、灰色立体控件、分段进度条和紧凑表格。进入主题后可在窗口工具栏切换四种皮肤：**Windows 3.1、Windows 2000、Windows XP、Mac OS X 10.6**；卡片、圆环、表格三种视图和明暗模式会分别保存在浏览器中。该适配不直接加载上游压缩产物，因为上游主题面向另一套 monitor-probe API。
