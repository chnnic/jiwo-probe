...
- 浅色／深色／自动明暗，以及完整／紧凑／列表显示模式均使用图标按钮；悬停可看名称，保留键盘操作和选中状态。LuminaPlus 与 Lite 的详情页「系统」新增 TCP／UDP 双线图，使用当前页面会话已收到的快照，每 30 秒最多采样一次，内存中最多保留最近 24 小时，刷新后重新累计；沿用 1／6／24 小时范围筛选。**目前主控没有提供连接数历史，所以不回填打开页面之前的数据**；离线、未上报与断流保留空档，真实零正常展示，不增加后台请求或本地持久化。
