...
- **TCP / UDP**：首页卡片显示整机连接数，沿用各主题配色。通用与 Lumina 卡片将下行、上行速度放在左侧，`TCP/UDP 数量/数量` 放在同一行右侧；Lumina 去掉速度旁的周期流量，今日流量与剩余额度保留。Glassmorphism、Emerald 将第三个到期/剩余价值小格替换为上下两排 TCP、UDP，并移除重复连接数行；页首总剩余价值和二级详情页保持不变。Premium 与极简卡片保留独立连接数行。TCP 是整机 `ESTABLISHED` 连接数，UDP 是整机 socket 数，**不是代理用户数**；真实零显示 `0`，旧版主控未上报显示 `—`。复用现有快照，不新增请求或提高刷新频率。
