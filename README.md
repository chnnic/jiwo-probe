...
- 页首采用 Lite 式紧凑布局，仅保留登录、主题选择、单个明暗切换按钮，手机下标题与操作区保持单行，过长标题省略。未手动覆盖时仍按主控主题名的默认明暗规则显示。完整／紧凑／列表显示模式继续使用图标按钮，保留悬停名称、键盘操作和选中状态。LuminaPlus 与 Lite 的详情页「系统」提供 TCP／UDP 双线图，直接读取主控 `probe-series?metric=system` 返回的 `tcp_connections`／`udp_connections` 历史，与 CPU／内存／网速复用同一次请求。支持 1／6／24 小时，对应 5／10／30 分钟桶平均值，保留小数；刷新后重新读取主控历史，**不再使用浏览器当前会话采样**。缺失数据和缺桶保留空档，真实零正常展示，不新增后台请求或本地持久化。需要主控 v0.5.5-beta.12 或更新版本，并开启连接数采集；旧主控或未上报时显示暂无历史，不用实时值伪造曲线。历史在主控内存中最多保留 24 小时，升级后需逐步积累。
...
- 延迟页提供目标标签多选、全选／全不选、延迟／丢包切换和各目标数值；系统页展示 CPU、内存、下行／上行速度和 TCP／UDP 四张趋势卡片，全部读取主控历史。两页支持 1／6／24 小时范围，图表自动适应宽度；网络速度跟随全局 bits／bytes 设置。系统信息仍在概览，缺失数据不补零。
...
- 解锁总数、分类按钮及折叠栏统一使用主控口径：所有返回的检测项（包括地区、CDN、Steam 货币等信息查询）计入总数；`yes` 与 `originals_only` 均计为已解锁。单项仍保留“仅自制剧”、信息查询、检测失败、未解锁等说明。首页锁图标全部解锁为金色、部分解锁为绿色、零解锁为灰色；无结果不算全解锁，也不显示按钮。三类计数之和与总计一致。
