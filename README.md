...
- 从右上角主题菜单选择 **Lite**；内部主题标识仍为 `mini`，兼容已有设置，支持浅色／深色。卡片、详细、列表三种视图独立记忆，不影响其他主题的视图偏好。
...
- Lite 的详情页采用六项标签切换：**概览 / 延迟 / 系统 / 流量 / 回程 / 解锁**，默认概览，只展示当前项目。概览保留续费、每 TB 费用和 TCP/UDP；解锁独立展示流媒体、AI、其他分类与数量，无需再次展开。详情与解锁弹窗外框高度统一为 `min(820px, 100dvh - 32px)`；顶部标签与关闭按钮固定，仅内容区滚动，其他主题不变。
- 延迟页提供目标标签多选、全选／全不选、延迟／丢包切换和各目标数值；系统页同时展示 CPU、内存、下行／上行速度三张趋势卡片。两页支持 1／6／24 小时历史，图表自动适应宽度；网络速度跟随全局 bits／bytes 设置。系统信息仍在概览，历史缺失不伪装成 0 或实时曲线。
- 网速跟随 `PROBE_NETWORK_SPEED_UNIT`；延迟组数和默认目标跟随现有 `PROBE_PING_*` 设置。仅消费现有快照，不新增轮询。
- 默认使用纯色面板、浅色进度条和轻量静态阴影，不使用卡片毛玻璃。**Lite 不显示许可证信息、名牌或许可证动画**；其他主题的许可证保持原样。若需要给 Lite 设置自定义背景，请在 `PROBE_BACKGROUND_THEMES` 中显式加入 `mini`（默认背景主题名单不包含它）。
